...
int executeLine(PCB *ptrNow)
{
    ...
    if (fgets(str, sizeof(str), Archivo) != NULL)
    {
        ...
        FLAG_TOKEN = executeTokens(ptrNow, str);
        refresh();
...
void printPCB(PCB *ptrNow, int y)
{
    move(y, 0);
    int argStatus = strlen(ptrNow->status);

    for (int i = 0; i < 16; i++)
    {
        if (i < argStatus)
            addch(ptrNow->status[i]);
        else
            addch(' ');
    }
    argStatus = strlen(ptrNow->archivoNombre);
    for (int i = 0; i < 16; i++)
    {
        if (i < argStatus)
            addch(ptrNow->archivoNombre[i]);
        else
            addch(' ');
    }
    printw(" %d", ptrNow->priority);
    addch('\t');
    addch('\t');
    printw(" %d", ptrNow->id);
    addch('\t');
    printw("%d\t", ptrNow->PC);
    argStatus = strlen(ptrNow->IR);
    for (int i = 0; i < 15; i++)
    {
        if (i < argStatus)
            addch(ptrNow->IR[i]);
...
void printExecution(PCB *ptrRunning)
{
    ...
    printw("STATUS\t\tFileName\tPriority\tPID\tPC\tIR\t\tAX\tBX\tCX\tDX\n");
    for (int i = 0; i < x; i++)
    {
        addch('-');
    }
    if (ptrRunning != NULL)
        printPCB(ptrRunning, getcury(stdscr));
    else
        printw("");
...
void printReady(PCB *ptrReady)
{
    ...
    if (ptr != NULL)
    {
        do
        {
            ...
            ptrAux = ptr;
            printPCB(ptrAux, getcury(stdscr));
            ptr = ptrAux->next;
